...
export default function ProjectDetailModal({ project, trigger }: ProjectDetailModalProps) {
  ...
  const formatCardString = (cardString: string) => {
  ...
  const getPackageInfo = (packageType: string) => {
    switch (packageType) {
      case "SHORT":
        return { name: "Short Clip", price: "$100", description: "30-60 second edited clip" };
      case "FULL":
        return { name: "Full Project + Shorts", price: "$500", description: "Complete hand analysis with multiple social media clips" };
      case "CREDITS":
        return { name: "Credits Package", price: "$2000", description: "25 credits for any package type" };
      default:
        return { name: packageType, price: "N/A", description: "Unknown package" };
    }
  };
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                 ...
                 <div>
                   ...
                   <p className="text-sm">{packageInfo.name} - {packageInfo.price}</p>
                 </div>
                 <div>
                   <label className="text-sm font-medium text-gray-500">Where Played</label>
                   <p className="text-sm">{project.wherePlayed || "Not specified"}</p>
                 </div>
                 <div>
                   <label className="text-sm font-medium text-gray-500">Stakes</label>
               ...
               {project.adminNotes && (
                 <div className="pt-2 border-t">
                   ...
                   <p className="text-sm bg-gray-50 p-2 rounded mt-1">{project.adminNotes}</p>
                 </div>
               )}
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">{project.user.name || "Not provided"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm font-mono text-xs">{project.user.id}</p>
                </div>
                <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="text-sm font-medium text-gray-500 mb-2 block">Your Hand</label>
                  <CardListDisplay cards={formatCardString(project.yourHand)} />
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500 mb-2 block">Opponent's Hand</label>
                  <CardListDisplay cards={formatCardString(project.opponentHand)} />
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500 mb-2 block">Flop</label>
                  <CardListDisplay cards={formatCardString(project.flop)} />
                </div>
                <div className="space-y-2">
                  <div>
                    <label className="text-sm font-medium text-gray-500 mb-2 block">Turn</label>
                    {project.turn ? (
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-500 mb-2 block">River</label>
                    {project.river ? (
              ...
              <Tabs defaultValue="voiceover" className="w-full">
                  ...
                  {project.voiceoverUrl ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Voiceover File</span>
                        <Badge variant="secondary">Audio</Badge>
                      </div>
                      <div className="p-4 border rounded-lg bg-gray-50">
                        {project.voiceoverUrl.startsWith('http') ? (
                          <div className="space-y-2">
                            <p className="text-sm text-gray-600">File uploaded to external service</p>
                            <a 
                          ...
                          <div className="space-y-2">
                            <p className="text-sm text-gray-600">File uploaded directly to server</p>
                            <audio controls className="w-full">
                              <source src={project.voiceoverUrl} type="audio/*" />
                ...
                <TabsContent value="video" className="space-y-4">
                  {project.videoUrl ? (
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-medium">Raw Video File</span>
